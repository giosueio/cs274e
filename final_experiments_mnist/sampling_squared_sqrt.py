...
# save real samples
svhn_3000_testsamples_np = svhn_3000_testsamples.cpu().squeeze().numpy()
mnist_3000_testsamples_np = mnist_3000_testsamples.cpu().squeeze().numpy()

for i in range(nsamples):
    plt.imsave('../real_samples/svhn/{}.png'.format(i), svhn_3000_testsamples_np[i], cmap='gray')
    plt.imsave('../real_samples/mnist/{}.png'.format(i), mnist_3000_testsamples_np[i], cmap='gray')


model_names = ['squared', 'sqrt']
SIs = [partial(PolynomialInterpolant, p=0.5), partial(PolynomialInterpolant, p=2)]
    ...
    with torch.no_grad():
        ...
        for b in tqdm(range(100)):
            ...
            for sample in gen_svhn_samples:
                plt.imsave(f'../gen_samples/{model_names[m]}/svhn/{i}.png', sample, cmap='gray')
                i+=1
            ...
            for sample in gen_mnist_samples:
                plt.imsave(f'../gen_samples/{model_names[m]}/mnist/{j}.png', sample, cmap='gray')
                j+=1
